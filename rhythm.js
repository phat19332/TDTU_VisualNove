  ...
  startBGM(onEndedCallback) {
    ...
    if (this.bgmBuffer) {
      ...
      if (typeof onEndedCallback === 'function') {
        this.bgmSource.onended = onEndedCallback;
      }
...
class NotesManager {
  ...
  loadBeatmap(beatmapData) {
    if (!beatmapData || !Array.isArray(beatmapData.notes)) {
      console.warn('Beatmap không hợp lệ, fallback sang beatmap rỗng.');
      beatmapData = { notes: [], baseSpeed: 600 };
    }
    ...
    this.notes.forEach(note => {
      ...
      if (diff <= 0 && lane.type === 'cpu') {
        if (note.type === 'hold') {
          ...
          // CPU hold handling: mark hit when full hold duration has passed
          if (-diff >= note.holdLength) {
            note.hit = true;
            note.isHolding = false;
            // Don't return — fall through so the tail can be cleaned up (hit=true will skip render)
          }
          // Don't return early here — let the render section draw the sustain tail
        } else {
          note.hit = true;
          lane.cpuIsHitting = true;
          cpuChar.playAnim(['left', 'down', 'up', 'right'][note.lane]);
      ...
      if (note.isHolding && lane.type === 'player') {
        ...
        // Tick mỗi 0.1s để tránh cộng điểm lặp theo FPS
        const tickBucket = Math.floor(note.holdProgress * 10);
        if (tickBucket !== note.holdTickBucket) {
          note.holdTickBucket = tickBucket;
          if (tickBucket >= 0 && tickBucket % 2 === 0) {
            gameLogic.addScore(5, 0.1);
          }
        }
      ...
      // Render Sustain Tail
      // Notes fall DOWNWARD (positive Y direction), so the tail extends BELOW the head.
      // holdPixelLen is the length of the tail in pixels.
      if (note.type === 'hold' && !note.hit && !note.miss) {
        const holdPixelLen = note.holdLength * finalSpeed;

        let tailStartY, tailEndY;

        if (note.isHolding) {
          // While actively holding: tail top is anchored at hitZoneY, tail bottom is at hitZoneY + remaining length
          const remainingLen = (note.holdLength - Math.abs(diff)) * finalSpeed;
          tailStartY = this.hitZoneY;
          tailEndY = this.hitZoneY + Math.max(0, remainingLen);
        } else {
          // Before being hit: head is at drawY, tail extends downward by holdPixelLen
          tailStartY = drawY;
          tailEndY = drawY + holdPixelLen;
        }
      ...
      // Render Head Note (only when not yet being held)
...
export class RhythmGame {
  ...
  normalizeBeatmap(inputBeatmap) {
    // Hỗ trợ cả format object { notes: [...] } và mảng notes thuần.
    if (Array.isArray(inputBeatmap)) {
      return { notes: inputBeatmap, baseSpeed: 600 };
    }
    if (inputBeatmap && Array.isArray(inputBeatmap.notes)) {
      return inputBeatmap;
    }
    return { notes: [], baseSpeed: 600 };
  }
  ...
  start(beatmapJson, bgmUrl, sfxUrl, onComplete) {
    ...
    this.pendingData = {
      beatmap: this.normalizeBeatmap(beatmapJson),
      bgm: bgmUrl,
      sfx: sfxUrl,
      cb: onComplete
    };
  ...
  async selectDifficulty(level) {
    ...
    if (!this.pendingData || !this.pendingData.beatmap) {
      console.warn('Thiếu beatmap pendingData, tạo fallback rỗng để tránh crash.');
      this.pendingData = {
        beatmap: { notes: [], baseSpeed: 600 },
        bgm: null,
        sfx: null,
        cb: null
      };
  ...
  _runEngine() {
    ...
    this.timeoutId = setTimeout(() => {
      ...
      // Không kết thúc game theo thời lượng file BGM vì có thể là file ngắn.
      // Vòng chơi sẽ kết thúc theo endTime (beatmap/tối thiểu 120s).
      this.audio.startBGM();
